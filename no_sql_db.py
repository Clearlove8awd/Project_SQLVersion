...
class Table():
    ...
    def update_user_messages(self,  username, new_message):
        '''
            Updates all messages sent by a user with the new message content
        '''
        for entry in self.entries:
            if entry[1] == username:
                entry[3] = new_message
...
class DB():
    ...
    def update_user_messages(self,  table_name, username, new_message):
        '''
            Updates all messages sent by a user with the new message content
        '''
        self.tables[table_name].update_user_messages(username, new_message)
...
database.create_table_entry('users', [user_id, "admin", hashed_password, [], publicKey, privateKey])
